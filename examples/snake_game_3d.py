...
import ModernGL
from miniglm import (Vec2, mat4_look_at,
                     mat4_perspective)
...
class Snake:
    ...
    def __init__(self):
        '''
            Constructor function which sets the length of the snake to 0, then
            creates the body, adding 3 parts to it, so the starter snake has the
            length of 3.
        '''
        self.length = 0
        self.body = []
    ...
    def initLength(self):
        '''
            Function, which adds the first bodypart to the body list. This
            function is required, because we place the head of the snake in the
            middle of the map at coordinates (25, 25).
        '''
        self.length += 1
        self.body.append({
            "pos" : Vec2((25, 25))
    ...
    def addLength(self):
        '''
            Function, which adds a bodypart to the body list and increases the
            length of the snake.
        '''
        self.length += 1
        self.body.append({
    ...
    def update(self, _x, _y):
        '''
            Function, which updates the position of each individual bodypart.
            Besides the head, each bodypart will get the position of the
            bodypart before it. The heads position is updated by the speeds on
            the oX and oY axes.
        '''
    ...
    def check(self):
        '''
            Function, which checks wether the head of the snake is colliding
